...
from siamtrack.pysot.utils.bbox import cxy_wh_2_rect
...
class SiamMaskTracker(SiamRPNTracker):
    ...
    def _mask_post_processing(self, mask):
        ...
        if len(contours) != 0 and np.max(cnt_area) > 100:
            ...
            prbox = np.array(cv2.boundingRect(polygon))
            prbox = np.array([prbox[0], prbox[1], prbox[0] + prbox[2], prbox[1] + prbox[3]])
            rbox_in_img = prbox
        else:  # empty mask
            ...
            rbox_in_img = np.array([location[0], location[1], location[0] + location[2], location[1] + location[3]])
    ...
    def track_zf(self, img, zf_list):
        ...
        res_mask_all = []
        for zf in zf_list:
            outputs = self.model.track_zf(x_crop, zf)
            score = self._convert_score(outputs['cls'])
            pred_bbox = self._convert_bbox(outputs['loc'], self.anchors)
            ...
            res_all.append([cx - width / 2, cy - height / 2, width, height, score[best_idx]])
            ...
            res_mask_all.append(polygon + [score[best_idx]])

        return np.array(res_all).reshape(-1, 5), np.array(res_mask_all).reshape(-1, 5)

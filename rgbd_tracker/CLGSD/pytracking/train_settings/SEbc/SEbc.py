...
'''2020.2.21 two-branch model, using same dataset as DiMP(not use trackingnet)'''
...
import ltr.models.SEbc.SEbc as SEbc
...
def run(settings):
    ...
    settings.max_gap = 50
    settings.sample_per_epoch = 4000 # 由于batchsize比原来更小了,所以我把这个值又翻了一倍 4000
    settings.sample_per_epoch_val = 500 # 500
    '''others'''
    ...
    # Create network
    net = SEbc.SEbc_resnet50(backbone_pretrained=True,used_layers=settings.used_layers,
    ...
    # Set objective
    objective = {}
    objective['bbox'] = IOULoss_mean(loss_type='giou') # GioU Loss
    objective['corner'] = nn.MSELoss() # take average of all elements


    # Create actor, which wraps network and objective
    actor = actors.SEbc_Actor(net=net, objective=objective)

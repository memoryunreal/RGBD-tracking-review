...
class DiMP(BaseTracker):
    ...
    def update_pos_sz(self, box):  # [x y x y]

        predicted_box = torch.Tensor(box.tolist()).float()
        predicted_box[2:] = predicted_box[2:] - predicted_box[:2] + 1  # [x y w h]
        # Get new position and size
        new_pos = predicted_box[:2] + predicted_box[2:] / 2
        new_pos = new_pos.flip((0,))
        new_target_sz = predicted_box[2:].flip((0,))
    ...
    def update(self, test_x, scale_ind, s, sample_pos, sample_scales, flag, update=True, refine_box=None):  # [x y w h]
        # ------- UPDATE ------- #
        if refine_box is None:
            ...
            # if update:
                ...
                #     target_box = self.get_iounet_box(self.pos, self.target_sz, sample_pos[scale_ind, :], sample_scales[scale_ind])
                # else:
                target_box = torch.Tensor(refine_box.tolist()).float()  # [x y w h]

                # Get new position and size
                new_pos = target_box[:2] + target_box[2:] / 2
                new_pos = new_pos.flip((0,))
                new_target_sz = target_box[2:].flip((0,))
                new_scale = torch.sqrt(new_target_sz.prod() / self.base_target_sz.prod())  # [h w]

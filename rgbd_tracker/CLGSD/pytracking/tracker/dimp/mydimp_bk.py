...
class DiMP(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        # Get target position and size   [cy cx h w]
    ...
    def re_initialize(self, image, info: dict) -> dict:
        self.frame_num = 1
    ...
    def track(self, image, update=True) -> dict:
        self.debug_info = {}
        ...
        # Localize the target      flag: uncertain / normal / hard_negative / not_found
        translation_vec, scale_ind, s, flag = self.localize_target(scores_raw, sample_scales)  # flag = None
        ...
        if flag not in ['not_found', 'uncertain'] and update:
            hard_negative = (flag == 'hard_negative')
            learning_rate = getattr(self.params, 'hard_negative_learning_rate', None) if hard_negative else None
            # Get train sample
            train_x = test_x[scale_ind:scale_ind+1, ...]
            # Create target_box and label for spatial sample   [cy cx h w] --> [x y w h]
        ...
        out = {'target_bbox': new_state.tolist(), 'max_score': max_score, 'debug_info': self.debug_info}
        return out

    def loc(self, image, siam_tracker) -> dict:
        self.debug_info = {}
        ...
        if flag != 'not_found':
            ...
            if siam_tracker is None:
                ...
                siam_tracker.tracker.center_pos = self.pos[[1,0]].detach().numpy()
                siam_tracker.tracker.size = self.target_sz[[1,0]].detach().numpy()
                local_out = siam_tracker.track(cv.cvtColor(image, cv.COLOR_RGB2BGR))  # {'box': out['target_bbox'], 'score': out['max_score']}
                refine_box = local_out['box']  # [x y w h] self.pos, self.target_sz

                # Get new position and size    [x y w h] --> [y x h w]
                new_pos = torch.from_numpy(refine_box[:2] + refine_box[2:] / 2)
                new_pos = new_pos.flip((0,))  # [y x]
                new_target_sz = torch.from_numpy(refine_box[2:]).flip((0,))
                new_scale = torch.sqrt(new_target_sz.prod() / self.base_target_sz.prod())  # [h w]
        ...
        # Compute output bounding box
        new_state = torch.cat((self.pos[[1,0]] - (self.target_sz[[1,0]]-1)/2, self.target_sz[[1,0]]))
        new_state[2:] = new_state[2:] + new_state[:2] - 1

        out = {'target_bbox': new_state.tolist(), 'max_score': max_score, 'debug_info': self.debug_info}
        return out, test_x, scale_ind, s, sample_pos, sample_scales, flag

    def update(self, test_x, scale_ind, s, sample_pos, sample_scales, flag, update=True, refine_box=None):
        # ------- UPDATE ------- #
        ...
        if flag not in ['not_found', 'uncertain'] and update:
            ...
            if refine_box is None:
                ...
                target_box = torch.from_numpy(refine_box).cuda().float()
            # Update the classifier model
            self.update_classifier(train_x, target_box, learning_rate, s[scale_ind, ...])

    def reset(self, box):  # [x y w h]
        x, y, w, h = box

        # Get target position and size   [x y w h] --> [cy cx h w]
        self.pos = torch.Tensor([y + (h - 1)/2, x + (w - 1)/2])
        self.target_sz = torch.Tensor([h, w])
    ...
    def localize_advanced(self, scores, sample_scales):
        ...
        sz = scores.shape[-2:]  # score [1, 19, 19]
        ...
        max_score1, max_disp1 = dcf.max2d(scores)  # max_val, max_idx/argmax
        _, scale_ind = torch.max(max_score1, dim=0)
        sample_scale = sample_scales[scale_ind]
        max_score1 = max_score1[scale_ind]
        max_disp1 = max_disp1[scale_ind,...].float().cpu().view(-1)
        target_disp1 = max_disp1 - score_center
        translation_vec1 = target_disp1 * (self.img_support_sz / self.feature_sz) * sample_scale  # (288 / 18)
        ...
        scores_masked = scores_hn[scale_ind:scale_ind + 1, ...].clone()  # score_hn = score
        scores_masked[...,tneigh_top:tneigh_bottom,tneigh_left:tneigh_right] = 0

        # Find new maximum
        max_score2, max_disp2 = dcf.max2d(scores_masked)  # max_val, max_idx/argmax
        ...
        if max_score2 > self.params.distractor_threshold * max_score1:  # 0.8 * score1
            ...
            return translation_vec1, scale_ind, scores_hn, 'uncertain'
        # score2 > 0.5 * score1 and score2 > 0.25
    ...
    def update_state(self, new_pos, new_scale = None):
        ...
        # Update pos     lim[-offset, offset]
        inside_ratio = getattr(self.params, 'target_inside_ratio', 0.2)
        inside_offset = (inside_ratio - 0.5) * self.target_sz
        self.pos = torch.max(torch.min(new_pos, self.image_sz - inside_offset), inside_offset)


    def get_iounet_box(self, pos, sz, sample_pos, sample_scale):  # [cy cx h w]
    ...
    def refine_target_box(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale = True):
        ...
        # Initial box for refinement    [y x h w] --? [x y w h]
        ...
        # Predict box
        k = getattr(self.params, 'iounet_k', 5)  # 3
        topk = min(k, output_boxes.shape[0])
        _, inds = torch.topk(output_iou, topk)
        predicted_box = output_boxes[inds, :].mean(0)  # [x y w h]
        predicted_iou = output_iou.view(-1, 1)[inds, :].mean(0)

        # Get new position and size    [x y w h] --> [y x h w]
        new_pos = predicted_box[:2] + predicted_box[2:] / 2
        new_pos = (new_pos.flip((0,)) - (self.img_sample_sz - 1) / 2) * sample_scale + sample_pos  # [y x]

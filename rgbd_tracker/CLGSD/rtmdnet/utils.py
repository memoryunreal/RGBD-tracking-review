...
def crop_image(img, bbox, img_size=[107, 107], padding=16, valid=False):
    ...
    scaled = cv2.resize(cropped, (img_size[0], img_size[1]))
    return scaled


def samples2maskroi(samples, receptive_field, cshape, padded_scene_size, padding_ratio=1.2):
    ...
    cur_resize_ratio = np.array(cshape) / np.array(padded_scene_size)

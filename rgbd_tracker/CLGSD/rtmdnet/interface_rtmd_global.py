...
class RTMD(object):
    ...
    def init(self, img, init_bbox, neg_examples):  # img [BGR] [x y w h]
        ...
        if neg_examples.shape[0] < self.opts['n_neg_init']:
            neg_examples2 = neg_examples.copy()
            while neg_examples2.shape[0] < self.opts['n_neg_init']:
                wh = neg_examples[:, 2:]
                cxy = neg_examples[:, :2] + wh / 2
                cxy = cxy + np.random.uniform(-0.15, 0.15) * wh
                wh = wh * np.random.uniform(0.96, 1.04)
                tmp_samples = np.concatenate((cxy - wh / 2, wh), axis=1)
                neg_examples2 = np.concatenate((neg_examples2, tmp_samples), axis=0)
                neg_examples2 = neg_examples2[overlap_ratio(neg_examples2, target_bbox, mode='xywh') < 0.5]
            neg_examples2 = np.random.permutation(neg_examples2)[:self.opts['n_neg_init']-neg_examples.shape[0]]
    ...
    def inference(self, img, target_bbox, proposals, thres=None):  # [x y w h] [x y x y]
    ...
    def collect(self, img, target_box, succ_flag, neg_s):  # [x y w h] [x y x y]
        cur_image = img[:, :, ::-1]  # BGR --> RGB
        ...
        if succ_flag:
            ...
            if neg_examples.shape[0] < self.opts['n_neg_update']:
                neg_examples2 = neg_examples.copy()
                while neg_examples2.shape[0] < self.opts['n_neg_update']:
                    wh = neg_examples[:, 2:]
                    cxy = neg_examples[:, :2] + wh / 2
                    cxy = cxy + np.random.uniform(-0.15, 0.15) * wh
                    wh = wh * np.random.uniform(0.96, 1.04)
                    tmp_samples = np.concatenate((cxy - wh / 2, wh), axis=1)
                    neg_examples2 = np.concatenate((neg_examples2, tmp_samples), axis=0)
                    neg_examples2 = neg_examples2[overlap_ratio(neg_examples2, target_bbox, mode='xywh') < 0.3]
                neg_examples2 = np.random.permutation(neg_examples2)[:self.opts['n_neg_update'] - neg_examples.shape[0]]
                neg_examples = np.concatenate((neg_examples2, neg_examples), axis=0)

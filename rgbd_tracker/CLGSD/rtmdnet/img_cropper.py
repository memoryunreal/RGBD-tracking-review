...
from rtmdnet.roi_align.modules.roi_align import RoIAlign
...
class imgCropper(nn.Module):
    def __init__(self, img_size):
        ...
        self.roi_align_model = RoIAlign(img_size, img_size, 1.0)
    ...
    def crop_image(self, image, box, result_size):
        ...
        cur_image_var = torch.from_numpy(cur_image_var.astype('float32'))

        roi = np.copy(box)
        roi[:, 2:4] += roi[:, 0:2]
        roi = np.concatenate((np.zeros((roi.shape[0], 1)), roi), axis=1)
        roi = torch.from_numpy(roi.astype('float32'))
    ...
    def crop_several_image(self, img_list, target_list):
        ...
        sample_rois = torch.from_numpy(sample_rois.astype('float32'))
        if self.isCuda:

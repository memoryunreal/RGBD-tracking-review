...
class SEcornernet(nn.Module):
    ...
    def __init__(self, feature_extractor, neck_module, head_module, used_layers, extractor_grad=True, unfreeze_layer3=False):
        ...
        """
        super(SEcornernet, self).__init__()
    ...
    def forward_test(self, test_imgs):
        ...
        corner_pred = self.head(fusion_feat)
        return corner_pred
...
@model_constructor
def SEcorner_resnet50(backbone_pretrained=True,used_layers=['layer3'],pool_size=None,unfreeze_layer3=False):
    # backbone
    backbone_net = backbones.resnet50(pretrained=backbone_pretrained)
    # neck
    neck_net = CorrNL.CorrNL(pool_size=pool_size)
    # head
    head_net = corner.Corner_Predictor(inplanes=pool_size*pool_size)

    net = SEcornernet(feature_extractor=backbone_net, neck_module=neck_net, head_module=head_net,
